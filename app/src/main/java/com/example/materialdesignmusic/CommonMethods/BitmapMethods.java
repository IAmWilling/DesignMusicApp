...
public class BitmapMethods {
     ...
     * @param radius 模糊半径
     * @param bitmap 传入位图对象
     * @return  返回已经设置好的高斯模糊的图片位图对象
     * */
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR1)
    public static Bitmap setBitmapGaussianBlur(Context context,Bitmap bitmap, float radius) {
        int width = Math.round(bitmap.getWidth() * BITMAP_SCALE);
        int height = Math.round(bitmap.getHeight() * BITMAP_SCALE);
        //Let's create an empty bitmap with the same size of the bitmap we want to blur
        Bitmap inBitmap = Bitmap.createScaledBitmap(bitmap,width, height ,false);
